...
+
  scale_x_discrete(limits= c(".00001",".00004"), labels(c('less than 1', '1 - 2', '2 - 3', '3 - 4', '5 or more' ))) 

+
  scale_fill_discrete(limits= c('less than 1', '1 - 2', '2 - 3', '3 - 4', '5 or more' ))

ggplot(data = brews.per.capita.map) + 
  ggtitle("Breweries Per Capita") +
  theme(plot.title = element_text(hjust = .5), legend.text = element) +
  geom_polygon(aes(x = long, y = lat, fill = factor(Brews.Per.Capita), group = group), color = "white") + 
  coord_fixed(1.3) + theme_nothing(legend = T)  +
  scale_color_manual(name = "Breweries per 100,000 People", labels= c('less than 1', '1 - 2', '2 - 3', '3 - 4', '5 or more') )
  

scale_x_discrete(limits= c(".00001",".00009")) +
  scale_fill_discrete(limits= c('less than 1', '1 - 2', '2 - 3', '3 - 4', '5 or more' ))
+ guides(fill=guide_legend(title = "Breweries per 100,000 People"))
```
...
p1 <- ggplot(BeerNBrew, aes(IBU, ABV)) + 
  ...
  theme(legend.position = "top", legend.text = element_text(size = 12), 
        legend.title = element_text(size = 14))+ggtitle("Correlation between IBU and ABV")
p1
```